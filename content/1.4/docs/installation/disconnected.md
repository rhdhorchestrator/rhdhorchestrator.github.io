...
yq e '.spec.install.spec.deployments[].spec.template.spec.containers[].image,
      ...
      select(.name | test("^RELATED_IMAGE_")).value' local-manifests-rhdh/rhdh-operator.clusterserviceversion.yaml
```
...
The list of images was obtained by:
```bash
podman create --name temp-container registry.redhat.io/rhdh-orchestrator-dev-preview-beta/orchestrator-operator-bundle:1.4-1741038322
